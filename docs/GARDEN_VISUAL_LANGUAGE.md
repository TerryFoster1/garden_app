# Garden Visual Language

The visual system should communicate garden state before text does.

## Environmental States

- Thriving: fresh green, steady glow, confident plant dots.
- Dry or hot: sunlight yellow, warm soil, heat/dry-edge indicators.
- Watch: coral accents, rings, warnings, and focused action.
- Shade: cool sage or blue-green overlays.
- Humid: pale sky or moisture tint with mildew watch language.

## Visual Components

- Garden hero: identity and ownership.
- Bed mini-map: spatial recognition.
- Plant dot: fast plant placement marker.
- Plant status ring: health/attention.
- Environmental pill: current microclimate state.
- Signal tile: sun/moisture/plant count/action context.
- Zone tabs: switch between beds, pots, indoor without deep menus.

## Text Reduction

Replace repeated metadata with:

- Icons.
- Status rings.
- Short chips.
- Environmental tinting.
- Mini maps.
- Focused labels.

Use text for decisions, not decoration.

## Plant Imagery Direction

Today, glyphs and visual plant marks are acceptable when they help recognition. They should never lead to dead placeholder pages. Future versions should become photo-driven:

- User plant photos.
- Growth photo thumbnails.
- Plant health scan snapshots.
- Harvest milestones.
- Seasonal state changes.

## Premium Feel

Premium does not mean more UI. It means fewer, clearer surfaces that feel alive, tactile, and trustworthy.

## Interaction States

Visual garden elements should imply action:

- Bed mini-maps open management.
- Plant dots open plant detail.
- Move chips relocate PlantInstances.
- Remove actions ask for confirmation.
- Add actions should preserve the selected bed/container/zone context.

The visual language should support recognition first, then action, then deeper reading.

## Planner Visuals

Planner visuals should show plant spacing and warning zones. A tomato should visually require more space than basil. Overlap and crowding should feel visible before the user reads a warning.
